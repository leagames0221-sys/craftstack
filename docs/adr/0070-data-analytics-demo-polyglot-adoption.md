...
- **No external API credentials.** `.env.example` ships placeholders only (deferred to T-03 when env vars first matter). Ollama runs locally; the narrative module asserts the absence of external-API env vars at invocation time (AC-4.3).
