...
> **Status (as of v0.5.17)**: this document was originally drafted in the design phase against ADRs 0001–0022 (the planned architecture). It has been reconciled with the **shipped implementation** — Pusher Channels instead of Fly.io + Socket.IO ([ADR-0052](../adr/0052-pusher-pivot-from-flyio-socketio.md) records the implementation-time pivot for [ADR-0046](../adr/0046-zero-cost-by-construction.md) compliance), hybrid retrieval (pgvector cosine kNN + Postgres FTS via RRF) shipped per [ADR-0063](../adr/0063-hybrid-retrieval-bm25-rrf.md) (closes ADR-0011 deferred), single-tenant → multi-tenant transition shipped per [ADR-0061](../adr/0061-knowlex-auth-and-tenancy.md), CI Credentials provider for Knowlex shipped per [ADR-0065](../adr/0065-knowlex-ci-credentials-provider.md). Knowlex live demo currently EMERGENCY_STOPPED post the 2026-04-29 Gemini Free tier account-level revocation incident — see [ADR-0067](../adr/0067-gemini-free-tier-account-revocation-incident.md); BYOK-reproducible per the README runbook.
...
`roleAtLeast()` is a pure helper with a 4×4 = 16-case Vitest matrix (`apps/collab/src/auth/rbac.test.ts`). `hasRole` / `requireRole` gate every mutation at the REST handler layer. Defense in depth: API checks + Prisma-layer membership checks + cross-workspace guards on set-mutations per [ADR-0029](../adr/0029-cross-workspace-guards.md). RLS at the DB layer was design-phase per ADR-0010 but deferred — both apps enforce membership application-side (Boardly via 4-tier RBAC + Membership; Knowlex via Auth.js + Membership + demo-allow-list per ADR-0061).
...
Boardly: workspace membership enforced at every REST handler via `requireWorkspaceMember`, with cross-workspace guards on set-mutations per [ADR-0029](../adr/0029-cross-workspace-guards.md). Knowlex: was single-tenant per [ADR-0039](../adr/0039-knowlex-mvp-scope.md) MVP scope; **workspace schema partitioning** (every table has `workspaceId NOT NULL`) shipped per [ADR-0047](../adr/0047-knowlex-workspace-tenancy-plan.md) partial in v0.5.0; the **access-control half** (Auth.js + `Membership` + demo-allow-list two-shape pattern, anonymous reads on `wks_default_v050` preserved + anonymous writes explicitly disallowed) shipped per [ADR-0061](../adr/0061-knowlex-auth-and-tenancy.md) v0.5.12 (closes I-01 honest disclose). PostgreSQL RLS per [ADR-0010](../adr/0010-rls-and-query-layer-defense.md) remains acknowledged-deferred — application-side enforcement was chosen for v0.5.12.
