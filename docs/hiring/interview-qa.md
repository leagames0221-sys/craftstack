...
Real eval cron numbers, not targets. The nightly `eval.yml` writes per-run reports to [`docs/eval/reports/YYYY-MM-DD.json`](../eval/) — each entry has `passRate`, `p95Ms`, and per-question latency. Run 3 (v3 substring-AND scoring) measured 19/30 = 63%. Run 6 measured 4/30 = 13.3% under stronger paraphrase scoring. Run 8 (Tuesday 2026-04-28, the first cron after v0.5.1 OR-mode + v0.5.2 ingest restoration) is the first stable measurement under v4 scoring, used for the v0.5.3 README badge. Latency targets per [ADR-0049](../adr/0049-rag-eval-client-retry-contract.md): `maxP95LatencyMs: 10000` (raised from 8000 in v0.5.1 to accommodate temperature + safety BLOCK_NONE generation overhead).
...
Unit (Vitest, **206 cases**: 166 collab + 40 knowledge — LexoRank, RBAC 4×4 matrix, validation, business logic, RAG retry contract, dedup, emergency stop, etc.), integration (Knowlex retrieve.integration.test.ts against a real `pgvector` service container in CI), E2E (Playwright, **24 scenarios**: smoke + authed E2E across board/dashboard/rate-limits/workspace + a11y + authed-a11y + signin), a11y (axe-core gate as PR-blocking on every public + authenticated page per [ADR-0034](../adr/0034-axe-core-a11y-in-playwright-smoke.md)), eval (nightly RAG cron with 30-question golden v4), drift-detect-v2 (`pg_catalog` assertion gating PRs per [ADR-0051](../adr/0051-prisma-migrate-on-vercel-build.md)). k6 scaffold exists but the realtime load harness is design-phase pending a Pusher-aware rewrite (Fly.io WebSocket-targeted version is non-runnable per ADR-0052).
