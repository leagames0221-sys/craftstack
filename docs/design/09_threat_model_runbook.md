...
| ID | 脅威 | 対策 |
|---|---|---|
| S-01 | セッション cookie 窃取 | HttpOnly + Secure + SameSite=Lax + __Secure- prefix + HSTS |
| S-02 | CSRF | Auth.js トークン + SameSite=Lax |
| S-03 | OAuth state fixation | Auth.js state/PKCE |
| S-04 | API キー推測 | Argon2 ハッシュ + klx_ prefix + 64 char random |
...
export const chatLimit = new Ratelimit({
  ...
  prefix: 'rl:chat:user', analytics: true,
})
